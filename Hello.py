...
def run():
        ...
        if job_description_file and resume_files:
                ...
                job_description_text = pdf_reader(job_description_bytes)
                ...
                for resume_file in resume_files:
                        ...
                        resume_text_truncated = resume_text[:500] + "..."
                        html = f'<html><head><style>.match-score{{font-size:24px;font-weight:bold;color:#488f31;}}.content{{padding:20px;}}</style></head><div class="content"><div><h4><b>Resume:</b> {resume_file.name}</h4></div><p>{resume_text_truncated}</p><div class="match-score">Match Score: {int(similarity*100)}%</div></div></html>'
